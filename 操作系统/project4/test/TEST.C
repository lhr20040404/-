...
void  MyTask (void *pdata)
{
	...
	PC_DispStr(10,++y,
			s1,
			DISP_BGND_BLACK+DISP_FGND_ //颜色1);
	...
	PC_DispStr(30,y,s,DISP_BGND_BLACK+DISP_FGND_ //颜色1);

	sprintf(s,"%d",MemInfo.OSNUsed);
	PC_DispStr(40,y,s,DISP_BGND_BLACK+DISP_FGND_ //颜色1);
	if(Times>4)
	{
		OSMemPut (				//释放内存块
			IntBuffer,			//内存分区的指针
...
void  YouTask (void *pdata)
{
    ...
    for (;;) 
    {
	PC_DispStr(10,++y,s2,DISP_BGND_BLACK+DISP_FGND_ //颜色2);                                           
	IntBlkPtr=OSMemGet (			//请求内存块
			IntBuffer,		//内存分区的指针
			&err			//错误信息
			);    	
	...
	sprintf(s,"%0x",MemInfo.OSFreeList);
	PC_DispStr(30,y,s,DISP_BGND_BLACK+DISP_FGND_ //颜色2);

	sprintf(s,"%d",MemInfo.OSNUsed);
	PC_DispStr(40,y,s,DISP_BGND_BLACK+DISP_FGND_ //颜色2);        
	OSMemPut (				//释放内存块
		IntBuffer,			//内存分区的指针
		IntBlkPtr			//待释放内存块的指针
		);  		
...
void  HerTask (void *pdata)
{
    ...
    for (;;) 
    {
	PC_DispStr(10,++y,s3,DISP_BGND_BLACK+DISP_FGND_ //颜色3);
	...
	sprintf(s,"%0x",MemInfo.OSFreeList);
	PC_DispStr(30,y,s,DISP_BGND_BLACK+DISP_FGND_ //颜色3);

	sprintf(s,"%d",MemInfo.OSNUsed);
	PC_DispStr(40,y,s,DISP_BGND_BLACK+DISP_FGND_ //颜色3);	
